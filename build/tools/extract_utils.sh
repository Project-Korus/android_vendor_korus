...
function setup_vendor() {
    ...
    export KORUS_ROOT="$3"
    if [ ! -d "$KORUS_ROOT" ]; then
        echo "\$KORUS_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$KORUS_ROOT/$OUTDIR" ]; then
        mkdir -p "$KORUS_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$KORUS_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$KORUS_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$KORUS_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$KORUS_ROOT"/vendor/korus/build/tools/smali/baksmali.jar
        export SMALIJAR="$KORUS_ROOT"/vendor/korus/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$KORUS_ROOT"/vendor/korus/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$LINEAGE_ROOT"/vendor/korus/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$KORUS_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$KORUS_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$KORUS_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$KORUS_ROOT"/vendor/korus/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$KORUS_ROOT"/"$OUTDIR"/radio
